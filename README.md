...
**Migration note:** `v0.4.2` requires Node.js 26. Node.js 22 and 24 users cannot upgrade; `v0.3.0` remains installable but is no longer supported.

Check the published command before adding it to Codex:

```bash
npx -y propotsdam-mcp@0.4.2 --version
```

Add the reviewed version to Codex:

```bash
codex mcp add proPotsdam -- npx -y propotsdam-mcp@0.4.2 serve
```
...
```bash
npx -y propotsdam-mcp@0.4.2 auth set
```
...
```bash
npx -y propotsdam-mcp@0.4.2 auth status
npx -y propotsdam-mcp@0.4.2 inbox list
npx -y propotsdam-mcp@0.4.2 records list --domain repair_status
npx -y propotsdam-mcp@0.4.2 records raw get REC-1 --json
npx -y propotsdam-mcp@0.4.2 files export REC-1 --output-dir /tmp/propotsdam-exports
npx -y propotsdam-mcp@0.4.2 actions list --kind form
npx -y propotsdam-mcp@0.4.2 actions prepare DMG-NEW --value description="Heizung bleibt kalt"
npx -y propotsdam-mcp@0.4.2 actions send cmdsend --attachment-file ./schaden.jpg --value msg_txt="Deckel defekt" --value TOPIC_IW_...="Abfallbehälter"
npx -y propotsdam-mcp@0.4.2 actions send save_partner --value phone_ref="+491234567"
npx -y propotsdam-mcp@0.4.2 writes list --domain repair_report
```

For a global installation, upgrade, or removal:

```bash
npm install --global propotsdam-mcp@0.4.2
